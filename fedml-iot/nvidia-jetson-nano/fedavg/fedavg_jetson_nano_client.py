...
def register(args, uuid):
    ...
    PARAMS = {'device_id': str_device_UUID, 'dataset_indexes': args.dataset_index}
...
if __name__ == '__main__':
    ...
    # start training
    trainer = FedAVGTrainer(client_ID, main_args.dataset_index, train_data_local_dict, train_data_local_num_dict, test_data_local_dict, train_data_num, device,
                            args, model_trainer)

    size = args.client_num_per_round + 1
    client_manager = FedAVGClientManager(args, trainer, rank=client_ID, size=size, backend="MQTT", dataset_index=main_args.dataset_index)
    client_manager.run()
