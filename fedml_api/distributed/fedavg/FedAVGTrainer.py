...
class FedAVGTrainer(object):
    ...
    def train(self, round_idx = None):
        ...
        print(colorama.Fore.LIGHTGREEN_EX + 'train() in FedAVGTrainer, dataset_index:', str(self.dataset_index), 'label:', str(self.dataset_index % 10) + ', round:' + str(round_idx) + colorama.Style.RESET_ALL)

        # print training data based on client's index
        # for i in range(len(self.train_local)):
        #     print(self.train_local[i][1])

        self.trainer.train(self.train_local, self.device, self.args, self.dataset_index % 10)

        weights = self.trainer.get_model_params()

        gradients = self.trainer.get_model_gradients()
    ...
    def test(self):
        # train data
        train_metrics = self.trainer.test(self.train_local, self.device, self.args, self.dataset_index % 10)
        train_tot_correct, train_num_sample, train_loss = train_metrics['test_correct'], \
                                                          train_metrics['test_total'], train_metrics['test_loss']

        # test data
        test_metrics = self.trainer.test(self.test_local, self.device, self.args, self.dataset_index % 10)
